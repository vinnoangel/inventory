...
$transID = $var3.time().$var1.$var4;
	...
	$fomat =$content2['regnof'];

 	$_SESSION['transID'] = $fomat.$transID;
...
<table style="border:0px solid;" cellspacing="0" cellpadding="3" width="100%">
    ...
    <tr>
        <td  style="padding-top:20px; padding-bottom:20px; font-size:10px; font-weight:bold;"> Transaction ID: &nbsp;&nbsp; <?php $transID = $_SESSION['transID']; echo $transID ?> &nbsp;&nbsp;&nbsp;&nbsp;  <?php echo date("d-m-Y h:m:s") ?></td>
    </tr>
 </table>
...
<table  cellspacing="0" cellpadding="3" width="100%" style="border:#000 solid 1px; font-size:11px;">
	...
	for($i=0; $i < $no; $i++){
		...
		if($item_id > 0){
			...
			$select_content2=("select * from items WHERE iid ='$item_id'");
			$content_result2= mysqli_query($db, $select_content2) or die (mysqli_error($db));
			$content2 = mysqli_fetch_assoc($content_result2);
			$num_chk2 = mysqli_num_rows ($content_result2);
			if($num_chk2 > 0){
				//if($content2["quantity"] >= $quantity){
					...
					mysqli_query($db, "insert into sales SET customer_id='$custID', item='$item', item_id = '$item_id', price='$price', quantity='$quantity', soldprice='$soldprice', transID='$transID', xdate='$xdate', user='$xID', status ='$status' ") or die(mysqli_error($db));
				...
				<?php if($status == 1){ 
					mysqli_query($db, "UPDATE items SET quantity='$nq' WHERE iid = '$item_id'") or die(mysqli_error($db));
				?>
					 <tr>
						<td width="80%" valign="top">  <?php echo $item." ".$quantity. " * ". $price ; ?> </td>
						<td><?php echo $total = $quantity * $price ?> </td>
					</tr>
				 <?php }  else{?>
					 <tr>
	...
	/*			}
					...
					$err = "The demand of ".  $quantity . " ". $content2["item"]. " cannot be sold".  " Only ".$content2["quantity"]." remain in stock ";
					$err = $quantity . " ". $content2["item"]. " cannot be sold".  " Only ".$content2["quantity"]." remain in stock ";
					echo '
					<script language="javascript">
						alert( "The demand of '.$err.'")
					</script>
					';		
				}*/
...
<?php
	...
	$transID = $_SESSION['transID'];
	mysqli_query($db, "insert into paymenthistory SET custID='$custID', transactions='$qtys', transtype='1', amt='$sumTotal', paid = '$cash', bal='$bal', transID='$transID', xdate='$xdate', user='$xID', payment_status ='$pmode' ") or die(mysqli_error($db));
